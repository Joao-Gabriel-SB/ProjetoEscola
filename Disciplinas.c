...
void IncluirAlunosDisciplina (*disciplina lista_disciplinas[], int *qtd_disciplina, int  int MatriculaAluno){
 ...
 printf("\t\t\t\tInclusão de Aluno\n\n");
    ...
    for (int i = 0; cod_incluir[i] != '\0'; i++) {
     ...
     for (int i = 0; i < *qtd_disciplina; i++) {
		...
		if (achou == 1){
			lista_disciplinas.AlunosMatriculados[lista_disciplinas.quantidadematriculados] = MatriculaAluno;
			lista_disciplinas.qtd_alunos++;
		}
